...
float WindowMax,WindowMin,RotateAngle; //ขอบเขตหน้าต่าง Rotate มุมหมุนที่Manual , Final มุมหมุนของ Auto (จริงๆ Rotate ไม่ต้องใช้แล้วก็ได้มาจากโค้ดเก่า)
#define dirPin 7
#define stepPin 8
...
void Motor(){
   ...
   if (Window == 360){
    ...
    RotateAngle =map(Gamma,270,450,-90,90); //-90 ถึง 90 ต้องไปวัดค่าเทียบมอเตอร์มาแก้
    stepper.moveTo(RotateAngle);
    stepper.runToPosition();
   }
   else{
    stepper.moveTo(-90); //ใส่ค่าแก้ตรงนี้ด้วย
    stepper.runToPosition();
   }
   }
    //เคสทิศ W
   if (Window == 90){
    ...
    WindowMax=180;
    if (Gamma<WindowMax && Gamma>WindowMin){
    RotateAngle =map(Gamma,0,180,-90,90); //-90 ถึง 90 ต้องไปวัดค่าเทียบมอเตอร์มาแก้
    stepper.moveTo(RotateAngle);
    stepper.runToPosition();
   }
   else{
   ...
   if (Window == 180){
    ...
    WindowMax=270;
    if (Gamma<WindowMax && Gamma>WindowMin){
    RotateAngle =map(Gamma,90,270,-90,90); //-90 ถึง 90 ต้องไปวัดค่าเทียบมอเตอร์มาแก้
    stepper.moveTo(RotateAngle);
    stepper.runToPosition();
   }
   else{
   ...
   if (Window == 270){
    ...
    WindowMax=360;
    if (Gamma<WindowMax && Gamma>WindowMin){
    RotateAngle =map(Gamma,180,360,-90,90); //-90 ถึง 90 ต้องไปวัดค่าเทียบมอเตอร์มาแก้
    stepper.moveTo(RotateAngle);
    stepper.runToPosition();
   }
   else{
...
void SDCARD(){
  String dataString = "";
      ...
      dataString += "\t";
      dataString += "Sun Position";
      dataString += String(Gamma);
      dataString += "\t"; 
      dataString +="Voltage: ";
